...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	for (int i = 0; i < boxList.size(); i++) {
		...
		int points = getMeshPointsInBox(mesh, node.points, boxList[i], pointsReturned);
		if (points > 0) {
			node.children.push_back(TreeNode(boxList[i], pointsReturned));
			if (points > 1) {
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	...
	if (node.box.intersect(ray, 0, INFINITY)) {
...
void Octree::draw(TreeNode & node, int numLevels, int level) {
	if (level >= numLevels) return;
	ofColor colorLevels[10] = {ofColor::red, ofColor::orange, ofColor::yellow, ofColor::green, ofColor::blue, ofColor::violet, ofColor::indigo, ofColor::white, ofColor::orange, ofColor::chocolate};
	ofSetColor(colorLevels[level]);
	drawBox(node.box);
	level++;
	for (int i = 0; i < node.children.size(); i++) {
